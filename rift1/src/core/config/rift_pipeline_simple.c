...
int rift_advanced_config_validate(const RiftAdvancedConfig *config) {
  ...
  if (config->worker_pools.default_pool_size >
      config->worker_pools.max_pool_size) {
  ...
  if (config->worker_pools.default_pool_size <
      config->worker_pools.min_pool_size) {
...
void rift_demo_pipeline_config(const RiftAdvancedConfig *config) {
  ...
  printf("   Mode: %s\n",
         ...
         : config->pipeline.mode == RIFT_PIPELINE_PARALLEL ? "Parallel"
                                                           : "Hybrid");
  printf("   Max parallel stages: %u\n", config->pipeline.max_parallel_stages);

  printf("\n🧵 Threading Configuration:\n");
  printf("   Model: %s\n",
         config->threading.model == RIFT_THREADING_MODEL_1_PARALLEL
             ? "Model 1 (True Parallelism)"
         : config->threading.model == RIFT_THREADING_MODEL_2_CONCURRENT
             ? "Model 2 (Shared-Core)"
             : "Hybrid");
