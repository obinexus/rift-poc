...
// === GOVERNANCE MACROS ===
#define GOVERNANCE_VALIDATE(ext)                                               \
  validate_governance_triangle(evaluate_r_extension_governance(ext))
#define GOVERNANCE_NORM(triangle)                                              \
  ((triangle)->attack_risk + (triangle)->rollback_cost +                       \
